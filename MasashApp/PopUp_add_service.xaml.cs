...
public partial class PopUp_add_service : ContentPage
{
	public PopUp_add_service()
	{
		InitializeComponent();
        Loaded += Page_Loaded;
    }
    ...
    public async void Add_service(object sender, TappedEventArgs e)
    {
        ...
        string id_service = await StaticData.API.POST("/add_service", new Dictionary<string, string>()
        {
        ...
        StaticData.selectedCategoryService.Items.Add(new Model_service_item()
        {
            Name = Entry_Name.Text,
            Price = Convert.ToInt32(Entry_Price.Text),
            Time = Convert.ToInt32(Entry_Time.Text),
            PathImg = $"{StaticData.API.GetPathHttp("")}/{path}",
            LinkModelCategory = StaticData.selectedCategoryService,
        });
